  ...
  _createClass(LightGallery, [{
    ...
    value: function buildModules() {
      ...
      this.settings.plugins.forEach(function (plugin, index) {
        ...
        (function (num) {
          setTimeout(function () {
            _this2.plugins.push(new plugin(_this2, $LG));

            if (index === _this2.settings.plugins.length - 1) {
              _this2.modulesLoaded = true;
            }
          }, 10 * num);
        })(numberOfModules);
      });
    ...
    value: function openGallery() {
      ...
      if (!this.modulesLoaded) {
        setTimeout(function () {
          _this4.openGallery(index, element);
      ...
      if (this.zoomFromOrigin && element) {
        this.currentImageSize = lg_utils.getSize(element, this.$lgContent, top + bottom, this.galleryItems[this.index].__slideVideoInfo && this.settings.videoMaxSize);
      ...
      setTimeout(function () {
        _this4.outer.addClass('lg-components-open');
      }, timeout);
      ...
      setTimeout(function () {
        ...
        if (_this4.zoomFromOrigin && transform) {
          var currentSlide = _this4.getSlideItem(index);

          currentSlide.css('transform', transform);
          setTimeout(function () {
            currentSlide.addClass('lg-start-progress lg-start-end-progress').css('transition-duration', _this4.settings.startAnimationDuration + 'ms');

            _this4.outer.addClass('lg-zoom-from-image');
          });
          setTimeout(function () {
        ...
        setTimeout(function () {
          _this4.$backdrop.addClass('in');

          _this4.$container.addClass('lg-show-in');
        }, 10); // lg-visible class resets gallery opacity to 1

        if (!_this4.zoomFromOrigin || !transform) {
          setTimeout(function () {
            _this4.outer.addClass('lg-visible');
          }, _this4.settings.backdropDuration);
        } // initiate slide function


        _this4.slide(index, false, false, false);

        _this4.LGel.trigger(lGEvents.afterOpen);
      });
    ...
    value: function hideBars() {
      ...
      setTimeout(function () {
        _this5.outer.removeClass('lg-hide-items');

        if (_this5.settings.hideBarsDelay > 0) {
          _this5.outer.on('mousemove.lg click.lg touchstart.lg', function () {
            _this5.outer.removeClass('lg-hide-items');

            clearTimeout(_this5.hideBarTimeout); // Timeout will be cleared on each slide movement also

            _this5.hideBarTimeout = setTimeout(function () {
              _this5.outer.addClass('lg-hide-items');
            }, _this5.settings.hideBarsDelay);
          });

          _this5.outer.trigger('mousemove.lg');
        }
    ...
    value: function onLgObjectLoad($el, index, delay, speed, dummyImageLoaded) {
      ...
      $el.find('.lg-object').first().on('load.lg', function () {
        _this6.handleLgObjectLoad($el, index, delay, speed, dummyImageLoaded);
      });
      setTimeout(function () {
    ...
    value: function handleLgObjectLoad($el, index, delay, speed, dummyImageLoaded) {
      ...
      setTimeout(function () {
        ...
        if (!dummyImageLoaded) {
          _this7.LGel.trigger(lGEvents.slideItemLoad, {
            index: index,
            delay: delay || 0
          });
        }
    ...
    value: function addSlideVideoInfo(items) {
      ...
      items.forEach(function (element, index) {
        element.__slideVideoInfo = _this8.isVideo(element.src, index);
      });
    }
    /**
    ...
    value: function loadContent(index, rec) {
      ...
      if (!$currentSlide.hasClass('lg-loaded')) {
        ...
        if (iframe) {
          ...
          setTimeout(function () {
            _this9.LGel.trigger(lGEvents.hasVideo, {
              index: index,
              src: src,
        ...
        if (!$currentSlide.hasClass('lg-loaded')) {
          setTimeout(function () {
            ...
            if (srcset || sources) {
              ...
              _this9.initPictureFill(_$img);
            }

            _this9.onLgObjectLoad($currentSlide, index, delay, _speed, true);

            var mediaObject = $currentSlide.find('.lg-object').first();

            if (lg_utils.isImageLoaded(mediaObject.get())) {
              _this9.loadContentOnLoad(index, $currentSlide, _speed);
            } else {
    ...
    value: function loadContentOnLoad(index, $currentSlide, speed) {
      ...
      setTimeout(function () {
        ...
        _this10.outer.removeClass('lg-first-slide-loading');

        _this10.isDummyImageRemoved = true;
    ...
    value: function getItemsToBeInsertedToDom(index, prevIndex) {
      ...
      if (this.galleryItems.length <= 3) {
        this.galleryItems.forEach(function (_element, index) {
          itemsToBeInsertedToDom.push("lg-item-".concat(_this11.lgId, "-").concat(index));
        });
    ...
    value: function organizeSlideItems(index, prevIndex) {
      ...
      itemsToBeInsertedToDom.forEach(function (item) {
        if (_this12.currentItemsInDom.indexOf(item) === -1) {
          _this12.$inner.append("<div id=\"".concat(item, "\" class=\"lg-item\"></div>"));
        }
      });
    ...
    value: function makeSlideAnimation(direction, currentSlideItem, previousSlideItem) {
      ...
      setTimeout(function () {
        ...
        _this13.outer.addClass('lg-no-trans');

        _this13.outer.find('.lg-item').removeClass('lg-prev-slide lg-next-slide');
        ...
        setTimeout(function () {
          _this13.outer.find('.lg-item').removeClass('lg-current');

          currentSlideItem.addClass('lg-current'); // reset all transitions

          _this13.outer.removeClass('lg-no-trans');
        }, 50);
    ...
    value: function slide(index, fromTouch, fromThumb, direction) {
      ...
      if (!this.lgBusy) {
        ...
        setTimeout(function () {
          if (_this14.lGalleryOn) {
            _this14.loadContent(index, true);
          } // Add title if this.settings.appendSubHtmlTo === lg-sub-html


          if (_this14.settings.appendSubHtmlTo === '.lg-sub-html') {
            _this14.addHtml(index);
          }
        }, (this.lGalleryOn ? this.settings.speed + 50 : 50) + (fromTouch ? 0 : this.settings.slideDelay));
        setTimeout(function () {
          _this14.lgBusy = false;
          previousSlideItem.removeClass('lg-slide-progress');

          _this14.LGel.trigger(lGEvents.afterSlide, {
            prevIndex: prevIndex,
            index: index,
            fromTouch: fromTouch,
            fromThumb: fromThumb
    ...
    value: function touchEnd(endCoords, startCoords, event) {
      ...
      setTimeout(function () {
        _this15.$container.removeClass('lg-dragging-vertical');

        _this15.outer.removeClass('lg-dragging lg-hide-items').addClass('lg-components-open');

        var triggerClick = true;

        if (_this15.swipeDirection === 'horizontal') {
          distance = endCoords.pageX - startCoords.pageX;
          var distanceAbs = Math.abs(endCoords.pageX - startCoords.pageX);

          if (distance < 0 && distanceAbs > _this15.settings.swipeThreshold) {
            _this15.goToNextSlide(true);

            triggerClick = false;
          } else if (distance > 0 && distanceAbs > _this15.settings.swipeThreshold) {
            _this15.goToPrevSlide(true);

            triggerClick = false;
          }
        } else if (_this15.swipeDirection === 'vertical') {
          distance = Math.abs(endCoords.pageY - startCoords.pageY);

          if (_this15.settings.closable && _this15.settings.swipeToClose && distance > 100) {
            _this15.closeGallery();

            return;
          } else {
            _this15.$backdrop.css('opacity', 1);
          }
        }

        _this15.outer.find('.lg-item').removeAttr('style');

        if (triggerClick && Math.abs(endCoords.pageX - startCoords.pageX) < 5) {
          // Trigger click if distance is less than 5 pix
          var target = $LG(event.target);

          if (_this15.isPosterElement(target)) {
            _this15.LGel.trigger(lGEvents.posterClick);
          }
        }

        _this15.swipeDirection = undefined;
      }); // remove slide class once drag/swipe is completed if mode is not slide

      setTimeout(function () {
        if (!_this15.outer.hasClass('lg-dragging') && _this15.settings.mode !== 'lg-slide') {
          _this15.outer.removeClass('lg-slide');
        }
    ...
    value: function enableSwipe() {
      ...
      if (this.settings.enableSwipe) {
        this.$inner.on('touchstart.lg', function (e) {
          ...
          var $item = _this16.getSlideItem(_this16.index);

          if (($LG(e.target).hasClass('lg-item') || $item.get().contains(e.target)) && !_this16.outer.hasClass('lg-zoomed') && !_this16.lgBusy && e.targetTouches.length === 1) {
            isSwiping = true;
            _this16.touchAction = 'swipe';

            _this16.manageSwipeClass();
        ...
        this.$inner.on('touchmove.lg', function (e) {
          ...
          if (isSwiping && _this16.touchAction === 'swipe' && e.targetTouches.length === 1) {
            endCoords = {
              pageX: e.targetTouches[0].pageX,
              pageY: e.targetTouches[0].pageY
            };

            _this16.touchMove(startCoords, endCoords);

            isMoved = true;
          }
        });
        this.$inner.on('touchend.lg', function (event) {
          if (_this16.touchAction === 'swipe') {
            if (isMoved) {
              isMoved = false;

              _this16.touchEnd(endCoords, startCoords, event);
            } else if (isSwiping) {
              var target = $LG(event.target);
      ...
      if (this.settings.enableDrag) {
        this.outer.on('mousedown.lg', function (e) {
          ...
          if ($LG(e.target).hasClass('lg-item') || $item.get().contains(e.target)) {
            if (!_this17.outer.hasClass('lg-zoomed') && !_this17.lgBusy) {
              e.preventDefault();
              ...
              if (!_this17.lgBusy) {
                ...
                _this17.outer.get().scrollLeft += 1;
                _this17.outer.get().scrollLeft -= 1; // *

                _this17.outer.removeClass('lg-grab').addClass('lg-grabbing');

                _this17.LGel.trigger(lGEvents.dragStart);
              }
            }
          }
        });
        $LG(window).on("mousemove.lg.global".concat(this.lgId), function (e) {
          if (isDraging && _this17.lgOpened) {
            isMoved = true;
            endCoords = {
              pageX: e.pageX,
              pageY: e.pageY
            ...
            _this17.LGel.trigger(lGEvents.dragMove);
          }
        });
        $LG(window).on("mouseup.lg.global".concat(this.lgId), function (event) {
          ...
          if (isMoved) {
            ...
            _this17.LGel.trigger(lGEvents.dragEnd);
          } else if (_this17.isPosterElement(target)) {
          ...
          if (isDraging) {
            ...
            _this17.outer.removeClass('lg-grabbing').addClass('lg-grab');
          }
        });
      }
    }
      ...
      if (!this.lgBusy) {
        if (this.index + 1 < this.galleryItems.length) {
          ...
          if (_loop) {
            ...
            setTimeout(function () {
              _this18.outer.removeClass('lg-right-end');
            }, 400);
          }
        }
      }
      ...
      if (!this.lgBusy) {
        if (this.index > 0) {
          ...
          if (_loop) {
            ...
            setTimeout(function () {
              _this19.outer.removeClass('lg-left-end');
            }, 400);
          }
        }
      }
    ...
    value: function keyPress() {
      ...
      $LG(window).on("keydown.lg.global".concat(this.lgId), function (e) {
        if (_this20.lgOpened && _this20.settings.escKey === true && e.keyCode === 27) {
          e.preventDefault();

          if (_this20.settings.allowMediaOverlap && _this20.outer.hasClass('lg-can-toggle') && _this20.outer.hasClass('lg-components-open')) {
            _this20.outer.removeClass('lg-components-open');
          } else {
            _this20.closeGallery();
          }
        }

        if (_this20.lgOpened && _this20.galleryItems.length > 1) {
          if (e.keyCode === 37) {
            e.preventDefault();

            _this20.goToPrevSlide();
          }

          if (e.keyCode === 39) {
            e.preventDefault();

            _this20.goToNextSlide();
          }
        }
      });
    }
    ...
    value: function toggleMaximize() {
      ...
      this.getElementById('lg-maximize').on('click.lg', function () {
        _this23.$container.toggleClass('lg-inline');

        _this23.refreshOnResize();
      });
    }
  }, {
    ...
    value: function manageCloseGallery() {
      ...
      if (this.settings.closeOnTap) {
        ...
        this.outer.on('mousedown.lg', function (e) {
          ...
          if (_this24.isSlideElement(target)) {
            mousedown = true;
          } else {
            mousedown = false;
          }
        ...
        this.outer.on('mouseup.lg', function (e) {
          ...
          if (_this24.isSlideElement(target) && mousedown) {
            if (!_this24.outer.hasClass('lg-dragging')) {
              _this24.closeGallery();
            }
          }
        });
    ...
    value: function closeGallery(force) {
      ...
      setTimeout(function () {
        if (_this25.zoomFromOrigin && transform) {
          _this25.outer.removeClass('lg-zoom-from-image');
        }

        _this25.$container.removeClass('lg-show'); // Need to remove inline opacity as it is used in the stylesheet as well


        _this25.$backdrop.removeAttr('style').css('transition-duration', _this25.settings.backdropDuration + 'ms');

        _this25.outer.removeClass("lg-closing ".concat(_this25.settings.startClass));

        _this25.getSlideItem(_this25.index).removeClass('lg-start-end-progress');

        _this25.$inner.empty();

        if (_this25.lgOpened) {
          _this25.LGel.trigger(lGEvents.afterClose, {
            instance: _this25
          });
        }

        _this25.LGel.get().focus();

        _this25.lgOpened = false;
      }, removeTimeout + 100);
    ...
    value: function destroy() {
      ...
      setTimeout(function () {
        _this26.destroyModules(true);

        if (!_this26.settings.dynamic) {
          for (var index = 0; index < _this26.items.length; index++) {
            var element = _this26.items[index];
            var $element = $LG(element);
            $element.off("click.lgcustom-item-".concat($element.attr('data-lg-id')));
          }
        }

        $LG(window).off(".lg.global".concat(_this26.lgId));

        _this26.LGel.off('.lg');

        _this26.$container.remove();
      }, closeTimeout);
    }
  }]);
